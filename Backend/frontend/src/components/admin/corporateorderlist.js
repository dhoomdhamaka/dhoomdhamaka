...
function Orderslist(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`api/adminuserlist/corporatepointsvoucher/${forms[0].userId}`)
      .then((res) => {
        setdatapoints(res.data[0].points);
        setdatavoucher(res.data[0].voucher);

        axios
          .get(`api/corporateEvent/corporateInfo/${forms[0].orderId}`)
          .then((res) => {
  ...
  function cancelorder(value) {
    if (value === "Accepted") {
      axios
        .post(`api/eventInfo/corporateaccepted/${forms[0].orderId}`)
        .then((res) => {});
    }
    if (value === "Declined") {
      axios
        .post(`api/eventInfo/corporatedeclined/${forms[0].orderId}`)
        .then((res) => {});
    }
    if (value === "Refund") {
      axios
        .post(`api/eventInfo/corporaterefund/${forms[0].orderId}`, {
          refundamount,
        })
        .then((res) => {});
    }
  ...
  function status(value) {
    if (value === "venue") {
      axios
        .post(`api/adminuserlist/corporatechangevenue/${forms[0].orderId}`)
        .then((res) => {});
    }
    if (value === "decoration") {
      axios
        .post(`api/adminuserlist/corporatechangedecoration/${forms[0].orderId}`)
        .then((res) => {});
    }
    if (value === "photography") {
      axios
        .post(
          `api/adminuserlist/corporatechangephotography/${forms[0].orderId}`
        )
        .then((res) => {});
    }
    if (value === "catering") {
      axios
        .post(`api/adminuserlist/corporatechangecatering/${forms[0].orderId}`)
        .then((res) => {});
    }
  }
  function points(val) {
    axios
      .post(`api/adminuserlist/corporatepoints/${forms[0].userId}`, { val })
      .then((res) => {});
    window.location.reload();
  }
  function voucher(val) {
    axios
      .post(`api/adminuserlist/corporatevoucher/${forms[0].userId}`, { val })
      .then((res) => {});
    window.location.reload();
  }
