...
function Orderslist(props) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(`api/babyshower/babyshowerInfo/${forms[0].orderId}`)
          .then((res) => {
  ...
  function status(value) {
    if (value === "venue") {
      axios
        .post(`api/adminuserlist/babyshowerchangevenue/${forms[0].orderId}`)
        .then((res) => {});
    }
    if (value === "decoration") {
      axios
        .post(
          `api/adminuserlist/babyshowerchangedecoration/${forms[0].orderId}`
        )
        .then((res) => {});
    }
    if (value === "photography") {
      axios
        .post(
          `api/adminuserlist/babyshowerchangephotography/${forms[0].orderId}`
        )
        .then((res) => {});
    }
    if (value === "catering") {
      axios
        .post(`api/adminuserlist/babyshowerchangecatering/${forms[0].orderId}`)
        .then((res) => {});
    }
  }
  function points(val) {
